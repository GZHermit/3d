...
def start(args):
    ...
    dataset = ModelNet40(view_transform)
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()),  # Only finetunable params
                          lr=config.learning_rate,
    ...
    for epoch in range(last_epoch, config.max_epoch + 1):
        ...
        for i, data in enumerate(dataloader):
            view_data, label = data
            if config.cuda:
                view_data, label = view_data.cuda(), label.cuda()
            optimizer.zero_grad()
            pred = model(view_data)
            train_loss = MVCNN.get_loss(pred, label)  # because the dataparallel
            train_loss.backward()
            optimizer.step()
